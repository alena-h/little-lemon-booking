  ...
  switch (action.type) {
    ...
    case "RESERVE_TIME": {
      const updatedTimes = state.availableTimes.map((timeSlot) => {
        if (
          timeSlot.date.toDateString() === state.selectedDate.toDateString()
        ) {
          return {
            ...timeSlot,
...
const times = () => {
  return Array.from({ length: 6 }, (_, i) => ({
    value: String(i + 17) + ":00",
    disabled: Math.random() < 0.5,
  }));
};
var day_generator = Array.from({ length: 31 }, (_, i) => ({
  date: addDaysToDate(new Date(), i),
  times: times(),
}));
    ...
    <>
      ...
      <Routes>
        ...
        <Route
          path="/menu"
          exact
          element={<InProgress setCurrentPage={setCurrentPage} />}
        />
        <Route
          path="/order"
          exact
